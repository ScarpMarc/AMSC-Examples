...
namespace apsc
{
  ...
  public:
     ...
     * all processes may directly build the local matrix, for instance reading from a file.
     * In this case, for the setup we need just the number of rows and columns of the global
     * matrix
     */
    ...
    auto const & getLocalMatrix() const
    {
      return localMatrix;
    }
...
protected:
    ...
    Matrix localMatrix;
    std::vector<Scalar> localProduct;
    ...
    // I use mpi_typeof() in mpi_util.h to recover genericity. Note that to activate the overload
    // I need to create an object of type Scalar. I use the default constructor, withScalar{}
...
apsc::PMatrix<Matrix>::setup(const Matrix &gMat, MPI_Comm communic)
{
  ...
  localRandC[0].resize(mpi_size); // get the right size to avoid messing up things
  ...
  // I use for the partitioning the same block distribution type as the ordering of the matrix:
  // matrices in ROWMAJOR storage will be partitioned along rows and viceversa
  constexpr apsc::ORDERINGTYPE P_ORDERING=
      Matrix::ordering==ORDERING::ROWMAJOR?
  ...
  // broadcast. But rememeber that communication is costly.

  MatrixPartitioner<apsc::DistributedPartitioner,P_ORDERING>
  partitioner(global_nRows,global_nCols,mpi_size); // the partitioner
  ...
  MPI_Scatterv(gMat.data(),counts.data(),displacements.data(),MPI_Scalar_Type,
               localMatrix.data(),matrixSize,MPI_Scalar_Type,manager,mpi_comm);
}
...
apsc::PMatrix<Matrix>::product(const std::vector<Scalar> &x)
{
  ...
  if constexpr (Matrix::ordering==ORDERING::ROWMAJOR)
    ...
    {
      //  This case is much trickier. The local matrix has fewer columns than the rows of x,
      // so I need to reduce the global vector in input x
      // I need to get the portion of x corresponding to the columns in the global matrix
      ...
      // A more elegant, efficient (but complicated) version would be to use a view.
      // It could have been done if I've used Eigen matrices instead of vectors.
  ...
  if constexpr (Matrix::ordering==ORDERING::ROWMAJOR)
    {
      ...
      MPI_Gatherv(localProduct.data(),localProduct.size(),
                  MPI_Scalar_Type,
  ...
  else
    {
       ...
       */
      MPI_Reduce(localProduct.data(),v.data(),
                 global_nRows, MPI_Scalar_Type,
  ...
  if constexpr (Matrix::ordering==ORDERING::ROWMAJOR)
    {
      ...
      MPI_Allgatherv(localProduct.data(),localProduct.size(),
                     MPI_Scalar_Type,
                     v.data(),vec_counts.data(),vec_displacements.data(),
  ...
  else
    {
      ...
      MPI_Allreduce(localProduct.data(),v.data(),
                    global_nRows, MPI_Scalar_Type,
                    MPI_SUM,mpi_comm);
    }
